...
@ApiTags('DocumentsCatalog')
@ApiBearerAuth()
@Controller('documents-catalog')
export class DocumentsCatalogController {
  constructor(
    private readonly documentsCatalogService: DocumentsCatalogService,
  ) {}

  @Post()
  create(
    @Body() createDocumentsCatalogDto: CreateDocumentsCatalogDto,
    @Req() request: Request,
  ) {
    createDocumentsCatalogDto.createdById = request['user'].sub;
    const document: CreateDocumentsCatalogDto = {
  ...
  update(
    @Param('id') id: string,
    @Body() updateDocumentsCatalogDto: UpdateDocumentsCatalogDto,
    @Req() request: Request,
  ) {
